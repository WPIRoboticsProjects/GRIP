 ...
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
/** Operation to call {@link opencv_core#compare} */
public class Compare implements CVOperation {
    ...
    private final SocketHint<CmpTypesEnum> cmpopInputHint = new SocketHint.Builder(CmpTypesEnum.class).identifier("cmpop").initialValue(CmpTypesEnum.CMP_EQ).view(SocketHint.View.SELECT).domain(CmpTypesEnum.values()).build();
    ...
    public String getDescription() {
        return "Compare each pixel in two images using a given rule.";
    }

    @Override
    @SuppressWarnings("unchecked")
    public InputSocket<?>[] createInputSockets(EventBus eventBus) {
        return new InputSocket[] { new InputSocket(eventBus, src1InputHint), new InputSocket(eventBus, src2InputHint), new InputSocket(eventBus, cmpopInputHint) };
    }
    ...
    public void perform(InputSocket<?>[] inputs, OutputSocket<?>[] outputs) {
        ...
        final CmpTypesEnum cmpop = (CmpTypesEnum) inputs[2].getValue().get();
        try {
            opencv_core.compare(src1, src2, dst, cmpop.value);
