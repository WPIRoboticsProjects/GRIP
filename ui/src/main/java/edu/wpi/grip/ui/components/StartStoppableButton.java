...
public final class StartStoppableButton extends ToggleButton {
  ...
  private static final Image stopImage =
      new Image(StartStoppableButton.class.getResourceAsStream("/edu/wpi/grip/ui/icons/stop.png"));
  ...
  private String getButtonActionString() {
    return CaseFormat.UPPER_UNDERSCORE.converterTo(CaseFormat.UPPER_CAMEL)
        .convert(service.state().toString());
  }
  ...
  private List<String> getCurrentStyleClasses() {
    return Arrays.asList(BASE_STYLE_CLASS, service.isRunning() ? STARTED_STYLE_CLASS :
        STOPPED_STYLE_CLASS);
  }
