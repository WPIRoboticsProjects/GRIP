...
/**
 * A JavaFX control that represents a {@link Source}.  <code>SourceController</code>s are somewhat
 * analogous to {@link StepController}s in thatthe pipeline contrains them and they contain some
 * sockets, but <code>SourceController</code>s only have output sockets, and they show up in a
 * different place.
 *
...
public class SourceController<S extends Source> implements Controller {
  ...
  /**
   * An unmodifiable list of {@link OutputSocketController}s corresponding to the sockets that this
   * source produces.
   */
