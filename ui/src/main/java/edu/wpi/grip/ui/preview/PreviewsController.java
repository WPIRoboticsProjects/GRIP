...
public class PreviewsController {
   ...
   * previews. This method is static so that it doesn't rely on any member variables in order to
   * calculate the sort.
   *
   * @param a       The first socket in the comparison
   * @param b       The second socket in the comparison
   * @param steps   A snapshot of the steps. This should be exactly the same list for the entire
   *                sort.
  ...
  private static int compareSockets(OutputSocket<?> a, OutputSocket<?> b, ImmutableList<Step>
    ...
    if (a.getSource().isPresent() && b.getSource().isPresent()) {
      ...
      if (sourceA == sourceB) {
        ...
        // If both sockets are from sources, order them based on the order of the sources in the
        // pipeline
  ...
  private void sortPreviews(ObservableList<SocketPreviewView<?>> previews) {
    ...
    // Take a snapshot of the sources and the steps in the current state so the comparison won't
    // break.
    // If we don't take a copy of the lists before trying to sort them then any concurrent
