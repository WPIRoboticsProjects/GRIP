...
public final class ExceptionWitnessResponderButton extends Button {
  ...
  /**
   * If the popover hasn't been created before then this creates the popover. Otherwise it returns
   * the popover that was constructed the first time this method was run.
   *
  ...
  private synchronized ExceptionPopOver getPopover() {
    if (!popOver.isPresent()) {
      ...
      assert getScene() != null : "Scene should not be null, this object has not been added to "
          + "the scene yet";
      assert getScene().getRoot() != null : "Root should not be null, this object has not been "
          + "added to the scene yet";
  ...
  public void onExceptionEvent(ExceptionEvent event) {
    if (getScene() == null || getScene().getRoot() == null) {
      // We are responding to an event prior to this button being added to the scene. Ignore this
      // for now.
      return;
    }
