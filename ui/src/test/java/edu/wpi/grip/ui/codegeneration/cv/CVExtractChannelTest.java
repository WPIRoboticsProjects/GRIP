...
public class CVExtractChannelTest extends AbstractGenerationTest {
  ...
  boolean setup(int channel) {
    ...
    step.getInputSockets().get(1).setValue(new Integer(channel));
    return true;
  }

  @Test
  public void extractZeroTest() {
    test(() -> setup(0), (pip) -> validate(pip), "extractZeroTest");
  }
  ...
  void validate(PipelineInterfacer pip) {
    ...
    assertFalse("Pipeline output is empty", ((org.bytedeco.javacpp.opencv_core.Mat) out.get())
        .empty());
    Mat genMat = (Mat) pip.getOutput("CV_ExtractChannel0Output0", GenType.IMAGE);
