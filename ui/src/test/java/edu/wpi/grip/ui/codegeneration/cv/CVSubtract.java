...
public class CVSubtract extends AbstractGenerationTesting {
  ...
  boolean set() {
    ...
    Step sub = gen.addStep(opUtil.getMetaData("CV subtract"));
    gen.connect(imgOut, sub.getInputSockets().get(0));
    gen.connect(blur.getOutputSockets().get(0), sub.getInputSockets().get(1));
    return true;
  }
  
  @Test
  public void cvSubtractTest() {
    test(() -> set(), (pip) -> validate(pip), "CvSubtractTest");
  }
  ...
  void validate(PipelineInterfacer pip) {
    ...
    pip.process();
    Mat genMat = (Mat) pip.getOutput("CV_Subtract0Output0", GenType.IMAGE);
