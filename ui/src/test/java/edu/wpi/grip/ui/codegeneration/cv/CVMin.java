...
public class CVMin extends AbstractGenerationTesting {
  ...
  boolean set() {
    ...
    Step min = gen.addStep(opUtil.getMetaData("CV Min"));
    gen.connect(imgOut, min.getInputSockets().get(0));
    gen.connect(blur.getOutputSockets().get(0), min.getInputSockets().get(1));
    return true;
  }
  
  @Test
  public void cvMinTest() {
    test(() -> set(), (pip) -> validate(pip), "CvMinTest");
  }
  ...
  void validate(PipelineInterfacer pip) {
    ...
    pip.process();
    Mat genMat = (Mat) pip.getOutput("CV_Min_Output", GenType.IMAGE);
