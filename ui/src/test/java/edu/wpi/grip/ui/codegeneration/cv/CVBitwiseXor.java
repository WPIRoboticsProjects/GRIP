...
public class CVBitwiseXor extends AbstractGenerationTesting {
  ...
  boolean set() {
    ...
    Step xOr = gen.addStep(opUtil.getMetaData("CV bitwiseXor"));
    gen.connect(imgOut, xOr.getInputSockets().get(0));
    gen.connect(blur.getOutputSockets().get(0), xOr.getInputSockets().get(1));
    return true;
  }
  
  @Test
  public void cvBitwiseXorTest() {
    test(() -> set(), (pip) -> validate(pip), "CvBitwiseXorTest");
  }
  ...
  void validate(PipelineInterfacer pip) {
    ...
    pip.process();
    Mat genMat = (Mat) pip.getOutput("CV_Bitwise_Xor0Output0", GenType.IMAGE);
