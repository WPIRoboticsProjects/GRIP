...
public class CVScaleAdd extends AbstractGenerationTest {
  ...
  boolean setup() {
    ...
    Step sAdd = gen.addStep(opUtil.getMetaData("CV scaleAdd"));
    gen.connect(imgOut, sAdd.getInputSockets().get(0));
    gen.connect(blur.getOutputSockets().get(0), sAdd.getInputSockets().get(2));
  ...
  public void cvScaleAddTest() {
    test(() -> setup(), (pip) -> validate(pip), "CvScaleAddTest");
  }
  ...
  void validate(PipelineInterfacer pip) {
    ...
    pip.process();
    Mat genMat = (Mat) pip.getOutput("CV_ScaleAdd0Output0", GenType.IMAGE);
