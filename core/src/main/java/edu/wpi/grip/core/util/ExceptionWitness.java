...
public class ExceptionWitness {
  ...
  /**
   * Indicates to the witness that an exception has occurred. This will also post an {@link
   * ExceptionEvent} to the {@link EventBus}
   *
   * @param exception The exception that this is reporting. If the Exception is an
   *                  InterruptedException then this will not post an exception, instead, it will
   *                  set the threads interrupted state and return.
  ...
  /**
   * Allows a warning to be flagged without an exception. This should never be done when there is an
   * exception involved.
   *
  ...
  /**
   * Indicate that there isn't currently an exception. Clears the exception state and posts an
   * {@link ExceptionClearedEvent}. This method can be called every time that there isn't an
   * exception as an {@link ExceptionClearedEvent} will only be posted when there was previously an
   * exception flagged.
   */
