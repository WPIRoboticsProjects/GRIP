...
/**
 * An operation that publishes any type that implements {@link Publishable} to NetworkTables. To be
 * publishable, a type should have one or more accessor methods annotated with {@link PublishValue}.
 * This is done with annotations instead of methods
 *
 * @param <D> the type of the data to publish (e.g. {@link Number})
 * @param <P> the publishable analog of the data (e.g. {@link edu.wpi.grip.core.operations
 *            .network.NumberPublishable NumberPublishable})
 */
public class NTPublishAnnotatedOperation<D, P extends Publishable> extends
    PublishAnnotatedOperation<D, P> {

  /**
   * Creates an NTPublishAnnotatedOperation for a type that already implements {@link Publishable}
   ...
   * @param converter          function for converting instances of the input type to publishable
   *                           objects
   * @param publisherFactory   factory for creating the publisher
   */
  public NTPublishAnnotatedOperation(InputSocket.Factory inputSocketFactory,
  ...
  /**
   * Creates an {@code OperationDescription} for an {@link NTPublishAnnotatedOperation} that
   * publishes data of the given type.
   *
   * @param dataType the type of the data to publish
   */
