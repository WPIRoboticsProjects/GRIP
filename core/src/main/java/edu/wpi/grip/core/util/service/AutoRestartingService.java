...
public class AutoRestartingService<S extends Service> implements RestartableService {
   ...
   * @param policy  A policy that allows for custom logic to determine if a service that has entered
   *                the {@link State#FAILED} state should restart.
   */
  public AutoRestartingService(final Supplier<S> factory,
    ...
    this.delegateFactory = () -> {
      ...
      addListenersForReal(newDelegate, addedListeners, new RestartListener(), MoreExecutors
          .directExecutor());
      this.listeners.entrySet()
          .forEach(listenerExecutorEntry
              -> addListenersForReal(newDelegate, addedListeners, listenerExecutorEntry.getKey(),
              listenerExecutorEntry.getValue()));
      return newDelegate;
    };
  ...
  /**
   * This will create an AutoRestartingService that will always restart whenever the existing
   * service enters the {@link State#FAILED} state.
   *
   * @param factory This should always supply a new instance of the service that can be restarted.
   */
  public AutoRestartingService(final Supplier<S> factory) {
  ...
  /**
   * Adds the given listener to the service. If the service already contains the given listener it
   * wont be added again.
   *
   * @param service           The service to add the listeners to
   * @param existingListeners The existing set of listeners that have already been added to the
   *                          service.
   * @param listener          The listener to be added
   * @param executor          The executor to run the listener on.
   */
  private static void addListenersForReal(final Service service,
                                          final Set<Listener> existingListeners,
  ...
  private AutoRestartingService startAsync(boolean checkIfRunning) {
    synchronized (this) {
      ...
      if (checkIfRunning && !VALID_START_STATES.contains(currentState)) {
        throw new IllegalStateException("Can not start while in state " + currentState + ". Must "
            + "be in one of states: " + StringUtils.join(VALID_START_STATES, ", "));
  ...
  /**
   * This is only guaranteed to add the listeners to future services. It will make a best effort
   * attempt to add the listener to the existing service.
   */
  @Override
