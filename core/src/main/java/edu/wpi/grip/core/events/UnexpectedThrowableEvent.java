...
/**
 * Event should be thrown when Unexpected Throwable ends up in an {@link
 * Thread#uncaughtExceptionHandler}. This event may potentially only be handled once if the
 * exception is deemed fatal.
 */
public final class UnexpectedThrowableEvent implements LoggableEvent {
   ...
   * @param fatal     True if this cause the application to quit forcibly. If the throwable is an
   *                  {@link Error} than this is automatically true. Defaults to false.
   */
  ...
  /**
   * Handles the exception, this method can only be called once if the event is fatal. If the event
   * is fatal then this method will never return.
   *
   * @param handler Handles the exception but in the safest way possible. Will not be run if
   *                throwable is an interrupted exception.
   */
