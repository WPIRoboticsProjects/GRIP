...
/**
 * A {@link SocketHint} that's type is linked between many other sockets and who's type is defined
 * by whatever {@link InputSocket} was connected to it first.
 */
public final class LinkedSocketHint extends SocketHint.SocketHintDecorator {
  ...
  /**
   * @param inputSocketFactory  The factory for {@link InputSocket}.
   * @param outputSocketFactory The factory for {@link OutputSocket}.
   */
  @SuppressWarnings("unchecked")
  public LinkedSocketHint(InputSocket.Factory inputSocketFactory,
  ...
  public InputSocket<?> linkedInputSocket(String hintIdentifier) {
    return new LinkedInputSocket<>(inputSocketFactory.create(new
        IdentiferOverridingSocketHintDecorator(this, hintIdentifier)));
  }

  /**
   * Creates an input socket that is linked to this Socket This output socket will automatically be
   * disconnected when there is no longer an input socket to guarantee the type of this SocketHint.
   *
   * @param hintIdentifier The identifier for this socket's SocketHint
  ...
  public OutputSocket linkedOutputSocket(String hintIdentifier) {
    final OutputSocket outSocket = outputSocketFactory.create(new
        IdentiferOverridingSocketHintDecorator(this, hintIdentifier));
  ...
  public Class getType() {
    // If the type is known because one of the input sockets is connected then return that.
  ...
  private class LinkedInputSocket<T> extends InputSocket.Decorator<T> {
    ...
    public void removeConnection(Connection connection) {
      synchronized (this) {
        ...
        if (controllingSockets.isEmpty()) { // When the set is empty, the socket can support any
          // type again
