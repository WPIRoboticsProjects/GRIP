...
/**
 * An XStream converter that marshals and unmarshals {@link Connection}s. A marshalled connection
 * stores indexes indicating the output and input sockets it connects.
 */
public class ConnectionConverter<T> implements Converter {
  ...
  public Object unmarshal(HierarchicalStreamReader reader, UnmarshallingContext context) {
    ...
    // Read the child nodes of this connection. An output socket and an input socket should be
    // defined.
    ...
    // Send the new connection as an event, so the GUI and other classes can listen for new
    // connections even if regardless
    // of if they came from a serialized file or not.
    this.eventBus.post(new ConnectionAddedEvent(connectionFactory.create(outputSocket,
        inputSocket)));
    return null;
  }
