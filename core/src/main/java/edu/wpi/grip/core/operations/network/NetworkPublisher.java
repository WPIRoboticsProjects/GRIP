 ...
 * protocol implemented by a specific {@link Manager}. This class is designed to be in one of two
 * states, either the set of keys will be empty. In which case the name will be used as the publish
 * key. In the other case the keys list will be populated and each value will need to be published
 * with a specific key.
 *
...
public abstract class NetworkPublisher<T> implements AutoCloseable {
  ...
  /**
   * Sets the name for the publisher. It is acceptable to set the name to the existing name without
   * consequence. If the name for the publisher changes {@link #publishNameChanged(Optional,
  ...
  /**
   * Called when when the name assigned to the publisher changes. This method should not be called
   * directly, instead, call {@link #setName(String)}.
   *
