 ...
 * between the inputs and outputs of different steps. The pipeline class is responsible for
 * listening for other components of the application (such as the GUI) adding or removing steps and
...
public class Pipeline implements ConnectionValidator, SettingsProvider, StepIndexer {
   ...
   * complete then the lock unlocked again.
   *
   ...
   * If you do so you are making a poor design decision and should move whatever you are trying
   * to do into
   * this class.
   */
  ...
  /**
   * @return The unmodifiable set of connections between inputs and outputs of steps.
   */
  ...
  public boolean canConnect(OutputSocket<?> outputSocket, InputSocket<?> inputSocket) {
    ...
    // If both sockets are in steps, the output must be before the input in the pipeline.  This
    // prevents "backwards"
   ...
   * @param delta The distance to move it.
   */
  public synchronized void moveStep(Step step, int delta) {
    ...
    writeStepsSafelyConsume(steps -> {
      ...
      // Compute the new index of the step, clamping to the beginning or end of pipeline if it
      // goes past either end
