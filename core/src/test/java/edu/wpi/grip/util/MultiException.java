...
public final class MultiException extends RuntimeException {
  ...
  private MultiException(int size, Iterator<? extends Throwable> exceptions) {
    super(size + " exceptions were thrown. The first exception is listed as a cause.", exceptions
        .next());
   ...
   * that wraps it <li>Otherwise, return an instance of {@link MultiException} that wraps the first
   * exception in the {@code exceptions} collection. </ul> <p>Though this method takes any {@link
   * Collection}, it often makes most sense to pass a {@link java.util.List} or some other
   * collection that preserves the order in which the exceptions got added.
   *
