...
public class NoContextHandlerTest {
    ...
    public void testNoContext() throws IOException {
        ...
        assertEquals("NoContextHandler should have run and sent 404 status", 404, response.getStatusLine().getStatusCode());
        EntityUtils.consume(response.getEntity());

        response = sendHttpRequest("/someClaimedPath");
        assertEquals("ClaimingHandler should have run and sent 200 status", HttpServletResponse.SC_OK, response.getStatusLine().getStatusCode());
        EntityUtils.consume(response.getEntity());
    }
