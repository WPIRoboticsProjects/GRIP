...
public class PipelineRunnerTest {
  ...
  public static class SimpleTests {
    ...
    public void testRunEmptyPipelineSucceeds() {
      ...
      runner.stopAndAwait();
      assertEquals("Runner should have stopped in a timely manner", Service.State.TERMINATED,
          runner.state());
    }
    ...
    public void testRunningOperationThatThrowsExceptionWillNotPropagate() throws TimeoutException, InterruptedException {
      ...
      final OperationMetaData operationMetaData = new OperationMetaData(OperationDescription
          .builder().name("OperationThatThrowsExceptionOnPerform").build(), () -> operation);
      ...
      assertEquals("Exception event should have only run once", 1, exceptionEventReceiver
          .callCount);
      assertTrue("Exception event should have an exception", exceptionEventReceiver.event
          .getException().isPresent());
  ...
  public static class RunnerCounterTests {
    ...
    @Test
    public void testPipelineWontRunSourceIfStoppedAfterRunPipelineEvent() throws TimeoutException, InterruptedException {
      final Waiter sourceSupplierWaiter = new Waiter();
      final Waiter supplierBlockedWaiter = new Waiter();
      final PipelineRunner runner = new PipelineRunner(eventBus,
          () -> {
  ...
  static class RunCounterOperation implements SimpleOperation {
    private static final OperationDescription DESCRIPTION =
        OperationDescription.builder().name("Simple").build();
