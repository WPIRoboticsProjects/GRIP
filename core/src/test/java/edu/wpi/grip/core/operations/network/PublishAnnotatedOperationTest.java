...
public class PublishAnnotatedOperationTest {
  ...
  public void testNTValueOrder() {
    ...
    assertEquals("Wrong publish name", "Publish bar", sockets.get(2).getSocketHint()
        .getIdentifier());
    assertEquals("Wrong publish name", "Publish foo", sockets.get(3).getSocketHint()
        .getIdentifier());
  }
  ...
  @Test
  @SuppressWarnings({"unchecked", "PMD.NcssCount"})
  public void testPublishSimpleReportReturnsExpectedValues() {
    ...
    class TestMapNetworkPublisher<T> extends MapNetworkPublisher<T> {
      ...
      protected void doPublish(Map<String, T> publishMap) {
        assertTrue("The first key didn't get passed to the map",
            publishMap.containsKey(SimpleReport.KEY_1));
        assertTrue("The second key didn't get passed to the map",
            publishMap.containsKey(SimpleReport.KEY_2));
        doPublishRan[0] = true;
      }
    ...
    final TestPublishAnnotatedOperation<SimpleReport> testPublishAnnotatedOperation = new
        TestPublishAnnotatedOperation<>(SimpleReport.class, factory);
  ...
  static class TestPublishAnnotatedOperation<T extends Publishable> extends
      PublishAnnotatedOperation<T, T> {
