...
public class ProjectTest {
  ...
  public void setUp() throws Exception {
    ...
    pythonAdditionOperationFromSource = PythonScriptFile.create("import edu.wpi.grip.core.sockets"
        + " as grip\nimport java"
        + ".lang.Integer\n\nname = \"Addition Operation\"\n\ninputs = [\n    grip.SocketHints"
        + ".createNumberSocketHint(\"a\", 0.0),\n    grip.SocketHints.createNumberSocketHint("
        + "\"b\", 0.0),\n]\n\noutputs = [\n    grip.SocketHints.Outputs.createNumberSocketHint"
        + "(\"sum\", 0.0)," + "\n]\n\ndef perform(a, b):\n    return a + b\n")
        .toOperationMetaData(isf, osf);
  ...
  public void testSerializePipelineWithStepsAndConnections() throws Exception {
    final Step step1 = stepFactory.create(pythonAdditionOperationFromSource);
    final InputSocket<Number> a1 = (InputSocket<Number>) step1.getInputSockets().get(0);
    final InputSocket<Number> b1 = (InputSocket<Number>) step1.getInputSockets().get(1);
    final OutputSocket<Number> sum1 = (OutputSocket<Number>) step1.getOutputSockets().get(0);
