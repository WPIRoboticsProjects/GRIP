...
public final class ExceptionWitnessTest {
  ...
  public void testErrorSameOrigin() {
    fireAnError();
    assertTrue("The witness observer was not present", testWitnessListener.errorWitnessObserver
        .isPresent());
    assertEquals("The error run count was not 1", 1, testWitnessListener.errorRunCount);
    assertEquals("Witness observer was not the same as the observer from the error event",
        witnessObserver, testWitnessListener.errorWitnessObserver.get());
  }

  @Test
  public void testClearNoErrorCallCount() {
    errorWitness.clearException();
    assertEquals("No error was posted but clear fired a clear event", 0, testWitnessListener
        .clearRunCount);
  }
  ...
  public void testErrorThenClearCallCount() {
    fireAnError();
    errorWitness.clearException();
    assertTrue("The cleared witness observer was not present", testWitnessListener
        .clearedWitnessObserver.isPresent());
    assertEquals("The cleared run count was not 1", 1, testWitnessListener.clearRunCount);
    assertEquals("Witness observer was not the same as the observer from the cleared event",
        witnessObserver, testWitnessListener.clearedWitnessObserver.get());
  }
  ...
  public void testErrorThenClearMultipleCallCount() {
    ...
    assertEquals("The cleared run count was not 1 after multiple clearException calls without an "
        + "error call between them", 1, testWitnessListener.clearRunCount);
  }
