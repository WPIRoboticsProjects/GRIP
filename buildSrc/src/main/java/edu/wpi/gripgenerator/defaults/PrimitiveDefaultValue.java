...
/**
 * Default value for a primitive constant. If the opencv method defines that it takes a primitive
 * variable and the comment following defines the default is a primitive not an Enum Declaration
 * then this is used.
 */
public class PrimitiveDefaultValue extends DefaultValue {
   ...
   * @param defaultOverride Nullable, The default override to use instead of whatever the opencv
   *                        function defines should be used as a default.
   */
  ...
  public Expression getDefaultValue(String defaultValue) {
    ...
    switch (type.getType()) {
      ...
      case Char:
        assert defaultValue.length() == 1 : "Invalid default value: " + defaultValue + " for type" +
            " char";
