...
public class SocketHintDeclarationCollection {
   ...
   * @param paramTypes
   */
  public SocketHintDeclarationCollection(DefaultValueCollector collector, List<DefinedParamType>
      paramTypes) {
  ...
  private final void addToLists(DefinedParamType type, Map<Type, List<DefinedParamType>>
      assignmentMap, List<DefinedParamType> assignmentList) {
  ...
  private MethodCallExpr getValueExpression(String paramId, int index) {
    return new MethodCallExpr(getOrSetValueExpression(arrayAccessExpr(paramId, index), null),
        "get");
  }
    ...
    for (DefinedParamType paramType : paramTypes) {
      ...
      if (inputParamTypes.contains(paramType) && outputParamTypes.contains(paramType)) {
        if (paramType.getType().toStringWithoutComments().contains("Mat")) {
          assignments.add(
              generateCopyExpression(paramType, inputParamId, inputParamTypes.indexOf(paramType),
                  outputParamId, outputParamTypes.indexOf(paramType))
          );
        } else {
          throw new IllegalStateException("Can not generate Input/Output Socket for type: " +
              paramType.getType().toString());
        }
      ...
      if (paramType.getType() instanceof PrimitiveType && (!((PrimitiveType) paramType.getType())
          .getType().equals(PrimitiveType.Primitive.Boolean))) {
        ...
        switch (((PrimitiveType) paramType.getType()).getType()) {
          ...
          case Long:
            ...
            break;
          default:
            throw new IllegalStateException("Conversion for type " + paramType.getType() + " is " +
                "not defined");
        }
  ...
  public ObjectCreationExpr getSocketListParam(DefinedParamType definedParamType,
                                               ClassOrInterfaceType socketType, String
                                                   inputOrOutputPostfix) {
    //System.out.println("Generating for default " + (definedParamType.getDefaultValue()
    // .isPresent() ? definedParamType.getDefaultValue().get().getName().toString() : "null"));
    return new ObjectCreationExpr(null, socketType, Arrays.asList(
        new NameExpr("eventBus"),
        new NameExpr(definedParamType.getName() + inputOrOutputPostfix + SocketHintDeclaration
            .HINT_POSTFIX)
    ));
  }
  ...
  public BlockStmt getInputSocketBody() {
    return getInputOrOutputSocketBody(inputParamTypes, new ClassOrInterfaceType("InputSocket"),
        SocketHintDeclaration.INPUT_POSTFIX);
  }
  ...
  public BlockStmt getOutputSocketBody() {
    return getInputOrOutputSocketBody(outputParamTypes, new ClassOrInterfaceType("OutputSocket"),
        SocketHintDeclaration.OUTPUT_POSTFIX);
  }
  ...
  public List<SocketHintDeclaration> getAllSocketHints() {
    ...
    for (Map.Entry<Type, List<DefinedParamType>> type : inputHintsMap.entrySet()) {
      socketHintDeclarations.add(new SocketHintDeclaration(collector, type.getKey(), type
          .getValue(), DefinedParamType.DefinedParamDirection.INPUT));
    }
    for (Map.Entry<Type, List<DefinedParamType>> type : outputHintsMap.entrySet()) {
      socketHintDeclarations.add(new SocketHintDeclaration(collector, type.getKey(), type
          .getValue(), DefinedParamType.DefinedParamDirection.OUTPUT));
    }
