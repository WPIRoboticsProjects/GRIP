...
public class Operation {
  private static final ImportDeclaration OPERATION_IMPORT = new ImportDeclaration(new NameExpr
      ("edu.wpi.grip.core.operations.opencv.CVOperation"), false, false);
  private static final ImportDeclaration SOCKETS_IMPORT = new ImportDeclaration(new NameExpr("edu" +
      ".wpi.grip.core.sockets"), false, true);
  private static final ImportDeclaration EVENT_BUS_IMPORT = new ImportDeclaration(new NameExpr
      ("com.google.common.eventbus.EventBus"), false, false);
  private static final ImportDeclaration CV_CORE_IMPORT = new ImportDeclaration(new NameExpr("org" +
      ".bytedeco.javacpp.opencv_core"), true, true);
  private static final ClassOrInterfaceType iOperation = new ClassOrInterfaceType("CVOperation");
  private static final AnnotationExpr OVERRIDE_ANNOTATION = new MarkerAnnotationExpr(new NameExpr
      ("Override"));
  private static final AnnotationExpr SUPPRESS_ANNOTATION = new SingleMemberAnnotationExpr(new
      NameExpr("SuppressWarnings"), new StringLiteralExpr("unchecked"));
  ...
  private List<ImportDeclaration> getAdditionalImports() {
    List<ImportDeclaration> imports = new ArrayList(Collections.singletonList(new
        ImportDeclaration(new NameExpr("org.bytedeco.javacpp." + definedMethod
        .getParentObjectName()), false, false)));
  ...
  private MethodDeclaration getCategoryMethod() {
    ...
    getCategory.setBody(new BlockStmt(Collections.singletonList(new ReturnStmt(new NameExpr
        ("Category.OPENCV")))));
    return getCategory;
  }
  ...
  private Expression getFunctionCallExpression() {
    return new MethodCallExpr(
        ...
        operationParams.stream().map(DefinedParamType::getLiteralExpression).collect
            (Collectors.toList())
    );
  }
  ...
  private List<Statement> getPerformExpressionList(String inputParamId, String outputParamId) {
    ...
    List<Expression> expressionList = socketHintDeclarationCollection.getSocketAssignments
        (inputParamId, outputParamId);
    List<Statement> performStatement = expressionList.stream().map(ExpressionStmt::new).collect
        (Collectors.toList());
    ...
    performStatement.addAll(
        Arrays.asList(
            ...
            new ExpressionStmt(
                ...
                /*
                 * Afterwards iterate over all of the output sockets and call setValue using the
                 * value
                 * stored.
                 */
            new ForeachStmt(
