...
open class JpackageExec : DefaultTask() {
    ...
    @TaskAction
    fun exec() {
        // Build a Java 11 runtime image with jlink
        // Otherwise a Java 13 image would be generated by jpackage, which is not ideal given that Java 13 is not yet stable
        project.exec {
            println("Generating jlink runtime image")
            project.delete(project.buildDir.resolve("jlink"))
            setCommandLine(
                    ...
                    "--output",
                    project.buildDir.resolve("jlink").absolutePath,
                    "--no-header-files", "--compress=2", "--no-man-pages", "--strip-debug"
            )
        }
        ...
        project.exec {
            ...
            // Use jpackage from JDK 13 preview, but build a Java 11 image
            args.addAll("--runtime-image", project.buildDir.resolve("jlink").absolutePath)
